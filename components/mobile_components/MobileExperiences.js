...
export default function MobileExperiences() {
    ...
    return (
        <div id="mobile-experiences" className="flex md:hidden justify-center items-start w-full overflow-x-hidden">
            <div className="w-[95%] h-[95%] flex flex-col items-center justify-around pb-[20vw] pt-[5vw]">
                <div ref={titleRef} className="flex items-center w-full justify-center">
                    ...
                    <h2 className="w-full font-main text-[18vw] tracking-[0] leading-none text-center">EXPERIENCE</h2>
                </div>
                {experience_list.map((experience, idx) => {
                    return (
                        <Link href={`/experience/${idx}`} key={idx} className="flex flex-col justify-center  mt-4 mb-4 relative group duration-300 w-[80%]">
                            ...
                            <div className="flex justify-center group-hover:opacity-50 duration-300 w-full -z-20">
                                <TechCard techs={experience.techs} title={`"${experience.title}"`} isItalic={false} />
                            </div>
                        </Link>)
                })}
