...
export default function MobileProjects() {
    ...
    return (
        <div id="mobile-projects" className="flex md:hidden justify-center items-start w-full overflow-x-hidden">
            <div className="w-[95%] h-[95%] flex flex-col items-center justify-around pb-[20vw] pt-[5vw]">
                <div ref={titleRef} className="flex items-center justify-center w-full">
                    ...
                    <h2 className="w-full font-main text-[18vw] tracking-[3vw] leading-none">PROJECTS</h2>
                </div>
                {project_list.map((project, idx) => {
                    return (
                        <Link href={`/project/${idx}`} key={idx} className="flex flex-col justify-center  mt-4 mb-4 relative group duration-300 w-[80%]">
                            ...
                            <div className="flex justify-center group-hover:opacity-50 duration-300 w-full -z-20">
                                <TechCard techs={project.techs} title={`"${project.title}"`} isItalic={false} />
                            </div>
                        </Link>)
                })}
